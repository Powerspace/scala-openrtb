...
/**
  ...
  */
class OpenRtbBidSerde(nativeSerde: OpenRtbNativeSerde)(implicit er: ExtensionRegistry)
    extends EncoderProvider[BidResponse.SeatBid.Bid] {
  ...
  def decoder: Decoder[BidResponse.SeatBid.Bid] =
    er.decoderWithExtensions({ cursor =>
      for {
        ...
        qagmediarating <- cursor
          .downField("qagmediarating")
          .as[Option[Int]]
        ...
        BidResponse.SeatBid.Bid(
          ...
          wratio = wratio,
          hratio = hratio,
          admOneof = admOneof.getOrElse(AdmOneof.Empty)
        )
      }
    })
}
