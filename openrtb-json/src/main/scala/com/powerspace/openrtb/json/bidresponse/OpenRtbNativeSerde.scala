...
class OpenRtbNativeSerde(implicit er: ExtensionRegistry) extends EncoderProvider[NativeResponse] {
  ...
  /**
    ...
    * field afterwards.
    */
  implicit val assetsEncoder: Encoder[Asset] = (asset: Asset) =>
    extendedEncoder[Asset]
  ...
  private implicit val linkDecoder = er.decoderWithExtensions { cursor =>
    for {
  ...
  private implicit val titleDecoder = er.decoderWithExtensions { cursor =>
    for {
  ...
  private implicit val imgDecoder = er.decoderWithExtensions { cursor =>
    for {
  ...
  private implicit val videoDecoder = er.decoderWithExtensions { cursor =>
    for {
      vasttag <- cursor.downField("vasttag").as[String]
    } yield NativeResponse.Asset.Video(vasttag = vasttag)
  }

  private implicit val dataDecoder = er.decoderWithExtensions { cursor =>
    for {
  ...
  private implicit val eventTrackerDecoder = er.decoderWithExtensions { cursor =>
    for {
  ...
  private implicit val assetDecoder = er.decoderWithExtensions { cursor =>
    for {
  ...
  def unboxedEncoder =
    extendedEncoder[NativeResponse]
      .clean(toKeep = Seq("assets"))

  def decoder: Decoder[NativeResponse] = er.decoderWithExtensions { cursor =>
    for {
      ...
      NativeResponse(
        ver = str.orElse(int),
        assets = assets.getOrElse(Seq()),
        assetsurl = assetsurl,
        dcourl = dcourl,
        link = link,
        imptrackers = imptrackers.getOrElse(Seq()),
        jstracker = jstracker,
        eventtrackers = eventtrackers.getOrElse(Seq()),
        privacy = privacy
      )
    }
  }
