...
class OpenRtbUserSerde(implicit er: ExtensionRegistry) extends EncoderProvider[BidRequest.User] {
  ...
  private implicit val geoEncoder: Encoder[BidRequest.Geo] = new OpenRtbBidRequestSerde().geoEncoder
  ...
  private implicit val geoDecoder: Decoder[BidRequest.Geo] = new OpenRtbBidRequestSerde().geoDecoder
