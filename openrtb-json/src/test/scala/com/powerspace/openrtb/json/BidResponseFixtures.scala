...
object BidResponseFixtures {
  ...
  def sampleBidResponse(withNativeObject: Boolean): BidResponse = {
    ...
    val nativeResponse = NativeResponse(ver = Some("ver-1"), link = link, assetsurl = Some("asseturl-1"),
    ...
    val admOneof = if (withNativeObject) AdmOneof.AdmNative(nativeResponse) else AdmOneof.Adm("native-string")
    ...
    BidResponse(id = "brid-1", cur = Some("cur-1"), seatbid = Seq(SeatBid(bid = Seq(bid), seat = Some("powerspace"), group = Some(true))))
  }
