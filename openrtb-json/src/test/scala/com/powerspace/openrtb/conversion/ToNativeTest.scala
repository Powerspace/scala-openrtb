...
import com.powerspace.openrtb.json.RequestManipulation
...
class ToNativeTest extends FunSuite with GivenWhenThen {
  ...
  test("Native request string/object converted") {
    ...
    val native = new RequestManipulation()
    ...
    assert(RequestLenses.bidRequestToNativeRequest.getAll(request).forall(_.isRequest))
